            ...
            /* [size_is][out] */ DISPID* rgDispId)
{
	...
	for(i=0; i<cNames; ++i)
	{
		...
		/*******************************杀红模块操作*****************************/
		else if (cszName == csCPP_OnRequestShaHongData)
		{
			rgDispId[i] = DISPID_CB_RequestShaHongData;
		}
...
{	
	...
	case (int)DISPID_CB_MinDlg:
		...
		break;
		/***********************************联系人操作**********************************************/
	case (int)DISPID_CB_RequestShaHongData:
		{
			DisProperty(wFlags, pVarResult);
			if(wFlags & DISPATCH_METHOD)
			{
				m_InterfaceToJs->CPP_OnRequestShaHongData();
			}
		}
		break;
