...
void CDlgShiFaDingHong::InitListHeader()
{
	...
	{
		CString Str;
		Str.Format("杀特%02d",Index);
		m_ListCtrl.InsertColumn(Index,Str,	LVCFMT_CENTER,	nWidth); 
	}

	m_ListCtrl.InsertColumn(Count+1,_TEXT("统计杀V"),	LVCFMT_CENTER,	nWidth);
	m_ListCtrl.InsertColumn(Count+2,_TEXT("统计错误"),	LVCFMT_CENTER,	nWidth);
...
*/
			...
			if(Index > 1)
			{
			    ...
			    StrDing2+="杀 "+CDataManageCenter::GetDataStr(Temp,IsTrue,true);
				m_ListCtrl.SetItemText(Index,CountIndex, StrDing2);
				StrDing2.Empty();
				ShaV[Temp]++;
				Count++;
				CountIndex++;
		    ...
		    m_ListCtrl.SetItemText(Index,CountIndex, StrDing8);
			StrDing8.Empty();
			ShaV[Temp]++;
			Count++;
			if(!IsTrue)
		    ...
		    m_ListCtrl.SetItemText(Index,CountIndex, StrDing9);
			CountIndex++;
			...
			m_ListCtrl.SetItemText(Index,CountIndex+1, StrDing8);
			StrDing8.Empty();
