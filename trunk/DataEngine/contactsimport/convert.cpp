...
//将TrPbItem中的内容转换成card2.1格式的字符串。 
//当b的值为true时，写version属性，为false则不写。
int innerConvertTrPbItemtoVcard21(const TrPbItem& trPbItem, wstring & myvcard21str, bool bIsWriteVersion)
{
	...
	//分组名称
	if(trPbItem.nGroupID !=  -1)
	{
		pVProperty = new VProperty(L"GROUP");
		if(pVProperty != NULL)
		{
// 			char chGroupID[20];
// 			memset(chGroupID, 0, 20);
// 			sprintf(chGroupID, "%d", trPbItem.nGroupID);
...
/*******************************************************************************************************************************
* 将TrPbItem格式的数据转换成VCard2.1格式，并存存到指定的文件中。
********************************************************************************************************************************/
int ConvertTrPbItemtoVcard21andWriteToFile(const TrPbItem& trPbItem, WCHAR* pwchPath)
{
	// 生成数据流
	wstring str=L"\0";
	int rv = innerConvertTrPbItemtoVcard21(trPbItem, str, true);
	if (rv != 0)
	{
		return -1;
	}

	str = ReplaceWstr(str, L"&", L"&amp;");
	str = ReplaceWstr(str, L"<", L"&lt;");
...
/*******************************************************************************************************************************
* 将数组中TrPbItem格式的数据转换成VCard2.1格式，并存存到指定的文件中。
********************************************************************************************************************************/
int ConvertTrPbItemtoVcard21andWriteToOneFile(const vector< TrPbItem> &m_PbContactsMgr, WCHAR* path)
{
	wstring str=L"\0";
	wofstream fout;//=ofstream(L"d:\my.txt",wios::app);
	fout.open (path);
	...
	for(int i=0;i<num;i++)
	{
		int j=innerConvertTrPbItemtoVcard21(m_PbContactsMgr[i],str,false);
		if (j!=0)
			return j;
		const wchar_t * pch=str.c_str ();
		fout<<str<<L"\n";
...
/*******************************************************************************************************************************
* 将TrPbItem格式的数据转换成VCard3.0格式，并存存到指定的文件中。
********************************************************************************************************************************/
int ConvertTrPbItemtoVcard30andWriteToFile(const TrPbItem& trPbItem,WCHAR* path)
{
	wstring str=L"\0";
	ConvertTrPbItemtoVcard30(trPbItem, str);
	wofstream fout;//=ofstream(L"d:\my.txt",wios::app);
	fout.open (path);
	const wchar_t * pch=str.c_str ();
	fout<<str;
...
int innerConvertTrPbItemtoVcard30(const TrPbItem& pbitem,wstring& myvcard30str,bool b)
{
	myvcard30str.empty();
...
/*******************************************************************************************************************************
* 将TrPbItem格式的数据转换成VCard2.1格式的字符串。
********************************************************************************************************************************/
int ConvertTrPbItemtoVcard30(const TrPbItem& pbitem,wstring& myvcard30str)
{
	return innerConvertTrPbItemtoVcard30(pbitem,myvcard30str,false);
}

/*******************************************************************************************************************************
* 将数组中TrPbItem格式的数据转换成VCard3.0格式，并存存到指定的文件中。
********************************************************************************************************************************/
int ConvertTrPbItemtoVcard30andWriteToOneFile(const TrPbItem pbitem[],WCHAR* path,int num)
{
	...
	for(int i=0;i<num;i++)
	{
		int j=innerConvertTrPbItemtoVcard30(pbitem[i],str,false);
		if (j!=1)
			return j;
		const wchar_t * pch=str.c_str ();
		fout<<str<<L"\n";
			...
			else//为非转义字符，则为一个字段
			{
				...
				else if(ispara==true)//添加参数
				{
					...
					if(vp!=0)//在新建了一个性质的基础上，把参数加到性质中
					{
						...
						if(!strcmp(my,"CHARSET"))
						{
							if(!strcmp(paravalue,"UTF-8"))
								isutf8;
						}
						c2w(my1,512,my);
...
int ConvertVcard21toTrPbItem(const wstring & myvcard21str, TrPbItem* pTrPbItem)
{
	...
	if(pVProperty!=0)
	{
		...
		strcpy(pTrPbItem->company,value);
		...
		strcpy(pTrPbItem->departments,value);
	}
	...
	strcpy(pTrPbItem->birthday,value);
	...
	strcpy(pTrPbItem->duties,value);

	//组id
	pVProperty=0;
	memset(value, 0, 1500);pwchar2=0;
	pVProperty=pMyVcard->getProperty(L"GROUP");
	if(pVProperty!=0)
	{
		pwchar2=pVProperty->getValue(0);
		if(pwchar2!=0)
	...
	//pTrPbItem->nGroupID = atoi(value);
	strcpy(pTrPbItem->GroupIDs,value);
				...
				if(equals==0)//如果这一项是TEL
				{
					...
					if(numpara==0)		//-----phonenumber
					{
						...
						if(pwchar2!=0)
						{
							...
							strcpy(pTrPbItem->phonenumber,value);
							tel=true;
						}
					}
					else//参数个数>0时，就去遍历
					{
						...
						if(hascell==false&&hashome==true)//-----phonehome
						{
							...
							if(pwchar2!=0)
							{
								...
								strcpy(pTrPbItem->phonehome,value);
							}
						}
						else if(hascell==false&&haswork==true)//----phonework
						{
							...
							if(pwchar2!=0)
							{
								...
								strcpy(pTrPbItem->phonework,value);
							}
						}
						else if(hascell==true&&haswork==false&&hashome==false)//----mobilehome
						{
							...
							if(pwchar2!=0)
							{
								...
								strcpy(pTrPbItem->mobilenumber,value);
							}
						}
						if(hascell==true&&hashome==true)//----mobilehome为例防止有TEL;HOME;WORK;CELL的样式
						{
							...
							if(pwchar2!=0)
							{
								...
								strcpy(pTrPbItem->mobilehome,value);
							}
						}
						if(hascell==true&&haswork==true)//----mobilehome
						{
							...
							if(pwchar2!=0)
							{
								...
								strcpy(pTrPbItem->mobilework,value);
							}
						}
					...
					if(numpara>0)
					{
						...
						if(hashome==true)
						{
							...
							if(pwchar2!=0)
							{
								...
								strcpy(value,curp);
								delete[] curp; curp = NULL;
								strcpy(pTrPbItem->homeaddress,value);
								hashomeaddr=true;
								i=num;//停止循环
								//hashomeaddr=true;
							}
...
int ConvertVcard21toTrPbItemandReadFromFile(const WCHAR* path, TrPbItem* pTrPbItem)
{
	...
	return	ConvertVcard21toTrPbItem(str, pTrPbItem);
}
...
int ConvertVcard21toTrPbItemandReadFromFile(const WCHAR* path,vector< TrPbItem> &m_PbContactsMgr,int n)
{
	...
	while(!fin.eof())
	{
		...
		if(hasbegin==true&&(!strcmp(ch,"END:VCARD")||!strcmp(ch,"end:vcard")||!strcmp(ch,"END:VCARD\r")||!strcmp(ch,"end:vcard\r")))
		{
			...
			if(ind<n)
			{	
				TrPbItem pbitem=TrPbItem();
				ConvertVcard21toTrPbItem(str, &pbitem);
...
int ConvertVcard30toTrPbItemandReadFromFile(const WCHAR* path,TrPbItem& pbitem)
{
	...
	return	ConvertVcard30toTrPbItem(str,pbitem);
}
...
int ConvertVcard30toTrPbItemandReadFromFile(const WCHAR* path,TrPbItem pbitem[],int n)
{
	...
	while(!fin.eof())
	{
		...
		if(hasbegin==true&&(!strcmp(ch,"END:VCARD")||!strcmp(ch,"end:vcard")||!strcmp(ch,"END:VCARD\r")||!strcmp(ch,"end:vcard\r")))
		{
			c2w(wch,600,ch);	str+=wch;	str+=L"\n";
			if(ind<n)
			{	ConvertVcard21toTrPbItem(str,&pbitem[ind]);	ind++;	hasbegin=false;	}
			else return 3;//成功返回，当数组分配的空间不足
			str=L"";
		}
		else if(hasbegin==true&&(!strcmp(ch,"END:VCARD")||!strcmp(ch,"end:vcard")||!strcmp(ch,"END:VCARD\r")||!strcmp(ch,"end:vcard\r")))
...
//用于将TrPbItem结构转化为CSV时，对个值中含有的双引号，逗号进行处理
//用于处理TrPbItem结构中各个char型数组中有逗号存在时的情况，此时将其转换成引号 逗号，如果有引号，将其转换成引号引号的形式写入文件
...
/*******************************************************************************************************************************
* 将TrPbItem结构中的数据转换成CSV结构，并放在字符串中
********************************************************************************************************************************/
int ConvertTrPbItemtoCSV(const vector< TrPbItem*>* pVectorContacts, wstring& CSVstr)
{
	...
	CSVstr += L"姓名,手机号码(移动),手机号码(家庭),手机号码(办公),电话号码,电话号码(家庭),电话号码(办公),私人邮箱,家庭地址,公司名称,个人生日,传真,部门,职务,组\n";
	...
	for(int i = 0;i<num; i++) 
	{
		TrPbItem* pTrPbItem = pVectorContacts->at(i);
		if (pTrPbItem == NULL)
		{
			continue;
		}
		...
		c2w(wchBuffer, TR_MAX_CONTACT_ID_LENGTH, pTrPbItem->name);
		...
		// 手机号码(移动)
		memset(wchBuffer, 0, sizeof(WCHAR)*TR_MAX_CONTACT_ID_LENGTH);
		c2w(wchBuffer, TR_MAX_CONTACT_ID_LENGTH, pTrPbItem->mobilenumber);
		pwchBuffer = ConverCharToCsvData(wchBuffer);
		if (pwchBuffer)
		{
			CSVstr.append(pwchBuffer, wcslen(pwchBuffer));
		...
		// 手机号码(家庭)
		memset(wchBuffer, 0, sizeof(WCHAR)*TR_MAX_CONTACT_ID_LENGTH);
		c2w(wchBuffer, TR_MAX_CONTACT_ID_LENGTH, pTrPbItem->mobilehome);
		pwchBuffer = ConverCharToCsvData(wchBuffer);
		if (pwchBuffer)
		{
			CSVstr.append(pwchBuffer, wcslen(pwchBuffer));
		...
		// 手机号码(办公)
		memset(wchBuffer, 0, sizeof(WCHAR)*TR_MAX_CONTACT_ID_LENGTH);
		c2w(wchBuffer, TR_MAX_CONTACT_ID_LENGTH, pTrPbItem->mobilework);
		pwchBuffer = ConverCharToCsvData(wchBuffer);
		if (pwchBuffer)
		{
			CSVstr.append(pwchBuffer, wcslen(pwchBuffer));
		...
		// 电话号码
		memset(wchBuffer, 0, sizeof(WCHAR)*TR_MAX_CONTACT_ID_LENGTH);
		c2w(wchBuffer, TR_MAX_CONTACT_ID_LENGTH, pTrPbItem->phonenumber);
		pwchBuffer = ConverCharToCsvData(wchBuffer);
		if (pwchBuffer)
		{
			CSVstr.append(pwchBuffer, wcslen(pwchBuffer));
		...
		// 电话号码(家庭)
		memset(wchBuffer, 0, sizeof(WCHAR)*TR_MAX_CONTACT_ID_LENGTH);
		c2w(wchBuffer, TR_MAX_CONTACT_ID_LENGTH, pTrPbItem->phonehome);
		pwchBuffer = ConverCharToCsvData(wchBuffer);
		if (pwchBuffer)
		{
			CSVstr.append(pwchBuffer, wcslen(pwchBuffer));
		...
		// 电话号码(办公)
		memset(wchBuffer, 0, sizeof(WCHAR)*TR_MAX_CONTACT_ID_LENGTH);
		c2w(wchBuffer, TR_MAX_CONTACT_ID_LENGTH, pTrPbItem->phonework);
		pwchBuffer = ConverCharToCsvData(wchBuffer);
		if (pwchBuffer)
		{
			CSVstr.append(pwchBuffer, wcslen(pwchBuffer));
		...
		// 私人邮箱
		memset(wchBuffer, 0, sizeof(WCHAR)*TR_MAX_CONTACT_ID_LENGTH);
		c2w(wchBuffer, TR_MAX_CONTACT_ID_LENGTH, pTrPbItem->email);
		pwchBuffer = ConverCharToCsvData(wchBuffer);
		if (pwchBuffer)
		{
			CSVstr.append(pwchBuffer, wcslen(pwchBuffer));
		...
		// 家庭地址
		memset(wchBuffer, 0, sizeof(WCHAR)*TR_MAX_CONTACT_ID_LENGTH);
		c2w(wchBuffer, TR_MAX_CONTACT_ID_LENGTH, pTrPbItem->homeaddress);
		pwchBuffer = ConverCharToCsvData(wchBuffer);
		if (pwchBuffer)
		{
			CSVstr.append(pwchBuffer, wcslen(pwchBuffer));
		...
		// 公司名称
		memset(wchBuffer, 0, sizeof(WCHAR)*TR_MAX_CONTACT_ID_LENGTH);
		c2w(wchBuffer, TR_MAX_CONTACT_ID_LENGTH, pTrPbItem->company);
		pwchBuffer = ConverCharToCsvData(wchBuffer);
		if (pwchBuffer)
		{
			CSVstr.append(pwchBuffer, wcslen(pwchBuffer));
		...
		// 个人生日
		memset(wchBuffer, 0, sizeof(WCHAR)*TR_MAX_CONTACT_ID_LENGTH);
		c2w(wchBuffer, TR_MAX_CONTACT_ID_LENGTH, pTrPbItem->birthday);
		pwchBuffer = ConverCharToCsvData(wchBuffer);
		if (pwchBuffer)
		{
			CSVstr.append(pwchBuffer, wcslen(pwchBuffer));
		...
		// 传真
		memset(wchBuffer, 0, sizeof(WCHAR)*TR_MAX_CONTACT_ID_LENGTH);
		c2w(wchBuffer, TR_MAX_CONTACT_ID_LENGTH, pTrPbItem->fax);
		pwchBuffer = ConverCharToCsvData(wchBuffer);
		if (pwchBuffer)
		{
			CSVstr.append(pwchBuffer, wcslen(pwchBuffer));
		...
		// 部门
		memset(wchBuffer, 0, sizeof(WCHAR)*TR_MAX_CONTACT_ID_LENGTH);
		c2w(wchBuffer, TR_MAX_CONTACT_ID_LENGTH, pTrPbItem->departments);
		pwchBuffer = ConverCharToCsvData(wchBuffer);
		if (pwchBuffer)
		{
			CSVstr.append(pwchBuffer, wcslen(pwchBuffer));
		...
		// 职务
		memset(wchBuffer, 0, sizeof(WCHAR)*TR_MAX_CONTACT_ID_LENGTH);
		c2w(wchBuffer, TR_MAX_CONTACT_ID_LENGTH, pTrPbItem->duties);
		pwchBuffer = ConverCharToCsvData(wchBuffer);
		if (pwchBuffer)
		{
			CSVstr.append(pwchBuffer, wcslen(pwchBuffer));
		...
		//组ID
// 		char chGroupID[256];
// 		memset(chGroupID, 0, sizeof(chGroupID));
// 		sprintf(chGroupID, "%d", pTrPbItem->nGroupID);

		memset(wchBuffer, 0, sizeof(WCHAR)*TR_MAX_CONTACT_ID_LENGTH);
		c2w(wchBuffer, TR_MAX_CONTACT_ID_LENGTH, pTrPbItem->GroupIDs);
		pwchBuffer = ConverCharToCsvData(wchBuffer);
		if (pwchBuffer)
		{
			CSVstr.append(pwchBuffer, wcslen(pwchBuffer));
...
{
	int rv = 0;
	if (pchLineText == NULL || pTrPbItem == NULL)
	{
		return -1;
	}
	...
	char chTempbuff[TR_MAX_CONTACT_ID_LENGTH];
	...
	while(i != nTextLen)
	{
		...
		{
		case 0: // 姓名
			ConverCsvToCharData(pTrPbItem->name, chDataBuffer);
			break;
		case 1: // 手机号码(移动)
			ConverCsvToCharData(pTrPbItem->mobilenumber, chDataBuffer);
			break;
		case 2: // 手机号码(家庭)
			ConverCsvToCharData(pTrPbItem->mobilehome, chDataBuffer);
		    break;
		case 3: // 手机号码(办公)
			ConverCsvToCharData(pTrPbItem->mobilework, chDataBuffer);
		    break;
		case 4: // 电话号码
			ConverCsvToCharData(pTrPbItem->phonenumber, chDataBuffer);
			break;
		case 5: // 电话号码(家庭)
			ConverCsvToCharData(pTrPbItem->phonehome, chDataBuffer);
			break;
		case 6: // 电话号码(办公) 
			ConverCsvToCharData(pTrPbItem->phonework, chDataBuffer);
		    break;
		case 7: // 私人邮箱
			ConverCsvToCharData(pTrPbItem->email, chDataBuffer);
			break;
		case 8: // 家庭地址
			ConverCsvToCharData(pTrPbItem->homeaddress, chDataBuffer);
		    break;
		case 9: // 公司名称
			ConverCsvToCharData(pTrPbItem->company, chDataBuffer);
			break;
		case 10: // 个人生日 
			ConverCsvToCharData(pTrPbItem->birthday, chDataBuffer);
			break;
		case 11: // 传真
			ConverCsvToCharData(pTrPbItem->fax, chDataBuffer);
			break;
		case 12: // 部门
			ConverCsvToCharData(pTrPbItem->departments, chDataBuffer);
			break;
		case 13: // 职务
			ConverCsvToCharData(pTrPbItem->duties, chDataBuffer);
			break;
		case 14: // 组
			{
				memset(chTempbuff, 0, TR_MAX_CONTACT_ID_LENGTH);
				ConverCsvToCharData(pTrPbItem->GroupIDs, chDataBuffer);
			}			
			break;
		default:
		    break;
