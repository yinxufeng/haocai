...
void CDataManageCenter::SaveDataToCSVFile()
{
	...
	for(int Index =0; Index < m_ShuangSeQiuList.size(); )
	{
		...
		{
			CString FormatStr;
			FormatStr.Format("%02d: ",it->second.size());
			vector<int> TempData;
...
//搜索29组数据列表
vector<sData29> CDataManageCenter::SeartData29(sData29& SearchData,eSearch29Type SearchType)
{
	vector<sData29> JueDataList;
	
	vector<sData29> DingDataList;
	...
	case SEARCH_JUESHA:
		{
			...
			{
				int Count=0;
				for(int j = 0; j < m_Data29[i].m_JueSha.size(); j++)
				{
					bool IsWant=false;
					for(int k=0; k < SearchData.m_JueSha.size(); k++)
					{
						if(m_Data29[i].m_JueSha[j]  == SearchData.m_JueSha[k])
						{
							IsWant=true;
							break;
						}
				...
				{
					sSearchData29 Temp;
					Temp.m_SearchType = SEARCH_JUESHA;
					Temp.m_Data29 = m_Data29[i];
					TempDataList.push_back(Temp);
					
					//DataList.push_back(m_Data29[i]);
				}
			}
			break;                                    //按搜索绝杀号方式
		}
	case SEARCH_DINGDAN:
		{
			for(int i = 0; i < m_Data29.size(); i++)
			{
				...
				for(int j = 0; j < m_Data29[i].m_JueSha.size(); j++)
				{
					...
					{
						if(m_Data29[i].m_JueSha[j]  != SearchData.m_WeiHong[k])
						{
							IsWant=true;
							break;
						}
				...
				if(Count == SearchData.m_WeiHong.size())
				{
					sSearchData29 Temp;
					Temp.m_SearchType = SEARCH_JUESHA;
					Temp.m_Data29 = m_Data29[i];
			...
			for(int i = 0; i < m_Data29.size(); i++)
			{
				...
				if(JueCount == SearchData.m_JueSha.size() || DingCount == SearchData.m_WeiHong.size())
				{
					...
					if(JueCount == SearchData.m_JueSha.size())
						...
						Temp.m_SearchType = SEARCH_DINGDAN;
	...
	{
		if(TempDataList[i].m_SearchType == SEARCH_JUESHA)
		{
			JueShaCount++;
		}
		else if(TempDataList[i].m_SearchType == SEARCH_DINGDAN)
		{
			DingCount++;
		}
	}
