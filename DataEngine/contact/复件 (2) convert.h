...
//* 在sContactInfo结构中的条目中homeaddress一般设置为QUOTED-PRINTABLE，其他的一般都是用默认值。
...
/*********************************************************************************************************************
* 将sContactInfo格式的数据转换成VCard2.1格式
*
* @pbitem (in)为被转换的数据	sContactInfo 结构中homeaddress的结构是: 
...
/**********************************************************************************************************************
* 详细注解减函数ConvertsContactInfotoVcard21(.....);只是将这个函数生成的字符串写到指定的文件当中去了,
...
* @pbitem (in) sContactInfo数组； @path (in)要写入的文件的目录和文件名；	@enchset (in)说明略 @num (in)表示pbitem数组的维数。
* @ret 如果转换成功，返回CONVERT_OK，否则返回相应的错误代码

* 不建议使用这个函数。建议一个联系人写到一个文件中去，这样这个文件的兼容性更好。一个文件中写多个联系人的话有些程序可能会不识别。
***********************************************************************************************************************/
int ConvertsContactInfotoVcard21andWriteToOneFile(const vector< sContactInfo> &pbContactsMgr, WCHAR* path);

/**********************************************************************************************************************
* 将一个含有vcard2.1格式的字符串中的数据读到sContactInfo结构中
...
int ConvertsContactInfotoVcard30andWriteToFile(const sContactInfo& pbitem,WCHAR* path);
...
/**********************************************************************************************************************
* 这个结构体在函数ConvertCSVtosContactInfo(...)中使用，用于指定各个数据在CSV文件中的顺序。不同的csv文件各个数据排放的顺序可能不同
